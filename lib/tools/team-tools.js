...
export const teamTools = [
  ...
  },
  {
    ...
    parameters: {
      ...
      reason: {
        type: 'string',
        description: 'Why the user is aborting (recorded in state-history).',
        required: true,
      },
    },
    output: {
  ...
  },
  {
    ...
    parameters: {
      ...
      prompt: {
        type: 'string',
        description: 'What to ask the user (shown in the panel).',
        required: true,
      },
      contextRefs: {
        type: 'array',
        ...
        description: 'Optional artifact ids to attach.',
      },
      waitMinutes: {
        type: 'number',
        description: 'Override global default (10 min).',
      },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: () => [{ type: 'text', text: 'decision point opened' }] },
    async execute(args) {
  ...
  },
  {
    ...
    parameters: {
      ...
      feedback: {
        type: 'string',
        description: 'Optional free-text (constraints / corrections / extra info).',
      },
      isAdHoc: {
        type: 'boolean',
        description: 'True if responding to an ad-hoc DP (cosmetic; default false).',
      },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: `dp responded: action=${v.action}` }] },
    async execute(args) {
  ...
  },
  {
    ...
    output: { schema: { type: 'array', items: { type: 'object', additionalProperties: true } }, render: (_a, v) => [{ type: 'text', text: `${v.length} open dp(s)` }] },
    async execute(args) {
  ...
  },
  {
    ...
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: `step ${v.stepIndex} marked complete` }] },
    async execute(args) {
      if (typeof args.runId !== 'string' || args.runId.length === 0) {
  ...
  },
  {
    ...
    parameters: {
      ...
      feedback: {
        type: 'string',
        description: 'What needs to change ("需修改清单").',
        required: true,
      },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: `step ${v.stepIndex} marked failed` }] },
    async execute(args) {
      if (typeof args.runId !== 'string' || args.runId.length === 0) {
  ...
  },
  {
    ...
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: `branch ${v.memberId} complete` }] },
    async execute(args) {
      if (typeof args.runId !== 'string' || args.runId.length === 0) {
  ...
  },
  {
    ...
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: `branch ${v.memberId} failed` }] },
    async execute(args) {
      if (typeof args.runId !== 'string' || args.runId.length === 0) {
  ...
  // ---- P4: Plan + Artifact ----
  {
    ...
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: `plan ${v.id} created` }] },
    async execute(args) {
  ...
  },
  {
    ...
    output: { schema: { type: 'array', items: { type: 'object', additionalProperties: true } }, render: (_a, v) => [{ type: 'text', text: Array.isArray(v) ? v.length + ' item(s)' : JSON.stringify(v) }] },
    async execute(args) {
  ...
  },
  {
    ...
    parameters: {
      ...
      file: {
        type: 'string',
        description: 'Relative to <team-runs>/<run-id>/',
        required: true,
      },
      producedBy: {
        type: 'string',
  ...
  // is re-read on next register).
  {
    ...
    parameters: {
      runId: {
        type: 'string',
        description: 'Run that owns the artifact.',
        required: true,
      },
      artifactId: {
        type: 'string',
        description: 'The artifact id (without the run-id prefix).',
        required: true,
      },
      reason: {
        type: 'string',
        description: 'Why the artifact is being deleted (logged to state-history for audit).',
      },
    },
    output: {
      schema: {
  ...
  // ---- P5: 多 Team + 历史 ----
  {
    ...
    parameters: {
      ...
      modifiedTask: {
        type: 'string',
        description: 'Override the original task description (optional).',
      },
      injectArtifacts: {
        type: 'array',
        items: {
        ...
        description: 'Cross-Run artifact refs to inject.',
      },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
    async execute(args) {
  ...
  //   - 进程级 DSH 重启后的 reconcileOnBoot 标记的 run 也能直接走 resume
  {
    ...
    parameters: {
      runId: {
        type: 'string',
        description: 'The interrupted run to resume.',
        required: true,
      },
      reason: {
        type: 'string',
        description: 'Why the resume is happening (recorded in state-history).',
      },
    },
    output: {
      schema: {
  ...
  },
  {
    ...
    parameters: {
      ...
      limit: {
        type: 'number',
        description: 'Cap the number of returned runs (default 50).',
      },
    },
    output: { schema: { type: 'array', items: { type: 'object', additionalProperties: true } }, render: (_a, v) => [{ type: 'text', text: Array.isArray(v) ? v.length + ' item(s)' : JSON.stringify(v) }] },
    async execute(args) {
  ...
  // ---- P6: 成本纪律 ----
  {
    ...
    parameters: {
      ...
      extraRounds: {
        type: 'number',
        description: 'If user picks "continue", how many more rounds to allow.',
      },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
    async execute(args) {
  ...
  // ---- 2.0 A6: Role CRUD tools (P2 抛光; 2.0 #1 不变) ----
  //
  // These give agents a programmatic path to manage the global Role library.
  // The Form UI in `ui/team-config.js` (A4) is the user-facing surface;
  // these tools back the SKILL.md guided flow + headless / scripted use.
  {
    name: 'team.create_role',
    description:
      'Create a Role in the global library. Validates id format and adapter set; ' +
      'refuses to overwrite an existing id. See architecture §5.2.',
    parameters: {
      id: {
        type: 'string',
        description: 'Role id, must match /^[a-z0-9][a-z0-9._-]*$/.',
        required: true,
      },
      display_name: {
        type: 'string',
      ...
      cli_options: {
        type: 'object',
        additionalProperties: true,
        description: 'Adapter-specific CLI options.',
      },
      tools_allowed: {
        type: 'array',
        items: {
    ...
    output: {
      schema: {
        type: 'object',
        additionalProperties: true,
        properties: { role: { type: 'object', additionalProperties: true } },
      },
      render: (_a, v) => [{ type: 'text', text: `Role ${v.role.id} created.` }],
    },
    async execute(args) {
      const { create: createRole } = await import('../../services/role-service.js');
  ...
  },
  {
    ...
    description: 'Update an existing Role. The id is immutable; pass the new fields to replace.',
    parameters: {
      id: {
        type: 'string',
        required: true,
    ...
    output: {
      ...
      render: (_a, v) => [{ type: 'text', text: `Role ${v.role.id} updated.` }],
    },
    async execute(args) {
      const { update: updateRole } = await import('../../services/role-service.js');
      const { id, ...patch } = args;
  ...
  {
    name: 'team.delete_role',
    description:
      'Delete a Role. Refuses when a global Member still references this role ' +
      '(returns deleted=false with refUsers). No force flag — the caller must ' +
      'first remove or reassign the dependent Members.',
    parameters: {
      id: {
        type: 'string',
        required: true,
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          deleted: { type: 'boolean' },
          refs: { type: 'number' },
          refUsers: { type: 'array', items: { type: 'string' } },
        },
      },
      render: (args, value) => [
        { type: 'text', text: value.deleted ? `Role ${args.id} deleted.` : `Refused to delete role ${args.id}: ${value.refs} member(s) still reference it.` },
      ],
    },
    async execute(args) {
      const { remove: removeRole } = await import('../../services/role-service.js');
      const result = await removeRole(args.id);
      return { id: args.id, ...result };
    },
  },
  // ---- 2.0 A7: Member persistent CRUD tools ----
  {
    name: 'team.create_member',
    description:
      'Create a Member in the global library. Cross-validates the role_id ' +
      'against the existing roles (the role must exist before the member can).',
    parameters: {
      id: {
        type: 'string',
        description: 'Member id, must match /^[a-z0-9][a-z0-9._-]*$/.',
        required: true,
      },
      role_id: {
        type: 'string',
    ...
    output: {
      schema: {
        type: 'object',
        additionalProperties: true,
        properties: { member: { type: 'object', additionalProperties: true } },
      },
      render: (_a, v) => [{ type: 'text', text: `Member ${v.member.id} created.` }],
    },
    async execute(args) {
      const member = await memberService.create({
        ...
        persona: args.persona,
        adapter: args.adapter,
        cli_options_override: args.cli_options_override ?? {},
  ...
  },
  {
    ...
    description: 'Update an existing Member. The id is immutable.',
    parameters: {
      id: {
        type: 'string',
        required: true,
    ...
    output: {
      ...
      render: (_a, v) => [{ type: 'text', text: `Member ${v.member.id} updated.` }],
    },
    async execute(args) {
      const { id, ...patch } = args;
      const member = await memberService.update(id, patch);
      return { member };
    },
  },
  {
    ...
    description:
      'Delete a Member. Refuses when a TeamTemplate references the member or ' +
      'any in-flight run still uses it. No force flag — clean up references first.',
    parameters: {
      id: {
        type: 'string',
        required: true,
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          deleted: { type: 'boolean' },
          refs: { type: 'number' },
          refSources: { type: 'array', items: { type: 'string' } },
        },
      },
      render: (args, value) => [
        { type: 'text', text: value.deleted ? `Member ${args.id} deleted.` : `Refused to delete member ${args.id}: ${value.refs} reference(s).` },
      ],
    },
    async execute(args) {
      const result = await memberService.remove(args.id);
      return { id: args.id, ...result };
    },
  },
  // ---- 2.0 A8: TeamTemplate CRUD tools ----
  {
    name: 'team.create_template',
    description:
      'Create a TeamTemplate in the global library. Validates flow enum and ' +
      'member reference shape (member_id + instance_alias).',
    parameters: {
      id: {
        type: 'string',
        required: true,
    ...
    output: {
      ...
      render: (_a, v) => [{ type: 'text', text: `Template ${v.template.id} created.` }],
    },
    async execute(args) {
      const { create: createTemplate } = await import('../../services/team-template-service.js');
  ...
  {
    name: 'team.update_template',
    description: 'Update an existing TeamTemplate. The id is immutable.',
    parameters: {
      id: {
        type: 'string',
        required: true,
    ...
    output: {
      ...
      render: (_a, v) => [{ type: 'text', text: `Template ${v.template.id} updated.` }],
    },
    async execute(args) {
      const { update: updateTemplate } = await import('../../services/team-template-service.js');
      const { id, ...patch } = args;
  ...
  {
    name: 'team.delete_template',
    description:
      'Delete a TeamTemplate. Refuses when an in-flight (non-archived) run ' +
      'still references the template via `meta.json.template_id`. No force flag.',
    parameters: {
      id: {
        type: 'string',
        required: true,
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          deleted: { type: 'boolean' },
          refs: { type: 'number' },
          refRunIds: { type: 'array', items: { type: 'string' } },
        },
      },
      render: (args, value) => [
        { type: 'text', text: value.deleted ? `Template ${args.id} deleted.` : `Refused to delete template ${args.id}: ${value.refs} run(s) still reference it.` },
      ],
    },
    async execute(args) {
      const { remove: removeTemplate } = await import('../../services/team-template-service.js');
      const result = await removeTemplate(args.id);
      return { id: args.id, ...result };
    },
  },
];
