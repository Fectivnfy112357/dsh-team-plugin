...
export const teamTools = [
  ...
  // these tools back the SKILL.md guided flow + headless / scripted use.
  {
    ...
    parameters: {
      type: 'object',
      required: ['id', 'display_name', 'persona', 'adapter'],
      additionalProperties: false,
      properties: {
        id: { type: 'string', description: 'Role id, must match /^[a-z0-9][a-z0-9._-]*$/.' },
        display_name: { type: 'string' },
        persona: { type: 'string' },
        adapter: { type: 'string', enum: ['hermes', 'mcode', 'claude-code'] },
        cli_options: { type: 'object', additionalProperties: true, description: 'Adapter-specific CLI options.' },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        persona: { type: 'string' },
        adapter: { type: 'string', enum: ['hermes', 'mcode', 'claude-code'] },
        cli_options: { type: 'object', additionalProperties: true },
        tools_allowed: { type: 'array', items: { type: 'string' } },
        avatar: {
          type: 'object',
          required: ['color', 'shape'],
  ...
  // ---- 2.0 A7: Member persistent CRUD tools ----
  {
    ...
    parameters: {
      type: 'object',
      required: ['id', 'role_id', 'display_name', 'persona', 'adapter'],
      additionalProperties: false,
      properties: {
        id: { type: 'string', description: 'Member id, must match /^[a-z0-9][a-z0-9._-]*$/.' },
        role_id: { type: 'string' },
        display_name: { type: 'string' },
        persona: { type: 'string' },
        adapter: { type: 'string', enum: ['hermes', 'mcode', 'claude-code'] },
        cli_options_override: { type: 'object', additionalProperties: true },
  ...
  // ---- 2.0 A8: TeamTemplate CRUD tools ----
  {
    ...
    parameters: {
      type: 'object',
      required: ['id', 'name', 'flow', 'members'],
      additionalProperties: false,
      properties: {
        id: { type: 'string' },
        name: { type: 'string' },
        flow: { type: 'string', enum: ['handoff-round-table', 'pipeline-with-feedback', 'fan-out-collect'] },
        flow_config: { type: 'object', additionalProperties: true },
        members: {
          type: 'array',
          minItems: 1,
          items: {
