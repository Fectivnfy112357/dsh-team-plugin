...
export async function apply(ctx) {
  ...
  // 3e. Layout (top bar + footer) / sidebar / conversation / user-questions
  // / tool / plan slots (B2-B11). Each in its own try/catch so one
  // missing / broken module doesn't take the whole chrome down.
  const slotRegistrars = [
    ['layout', '../ui/layout.js', 'registerLayoutSlot'],
    ['sidebar', '../ui/sidebar.js', 'registerSidebarSlot'],
    ['user-questions', '../ui/user-questions.js', 'registerUserQuestionsSlot'],
    ['conversation', '../ui/conversation.js', 'registerConversationSlot'],
    ['tool', '../ui/tool.js', 'registerToolSlot'],
    ['plan', '../ui/plan.js', 'registerPlanSlot'],
  ];
  for (const [name, modulePath, fnName] of slotRegistrars) {
    try {
      const mod = await import(modulePath);
      if (typeof mod[fnName] === 'function') {
        mod[fnName](ctx);
      } else {
        ctx.logger.warn?.(`dsh-team-plugin: ${name} slot registrar ${fnName} missing`);
      }
    } catch (error) {
      ctx.logger?.warn?.(`dsh-team-plugin: ${name} slot registration skipped: ${error.message}`);
    }
  }
